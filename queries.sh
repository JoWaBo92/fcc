...
echo "$($PSQL "SELECT winners.name FROM games LEFT JOIN teams AS winners ON games.winner_id = winners.team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT opponents.name FROM games LEFT JOIN teams AS opponents on games.opponent_id = opponents.team_id WHERE year = 2014 AND round = 'Eighth-Final';")"
